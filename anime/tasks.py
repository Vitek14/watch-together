...
from .models import Anime, Genre, Studio, Poster, Screenshot, ScoreStat, Related, DateModel


logger = logging.getLogger(__name__)

# Подключаем бэкенд для результатов
results_backend = RedisBackend(url="redis://localhost:6379/1")


@dramatiq.actor
def update_anime_data(store_results=True):
    logger.info("Starting anime data update...")
    url = "https://shikimori.one/api/graphql"
    logger.debug(f"Requesting URL: {url}")
    payload = json.dumps({
        "query": "{\n  # look for more query params in the documentation\n  animes(limit: 50, season: \"2025\", order: popularity) {\n    id\n    malId\n    name\n    russian\n    licenseNameRu\n    english\n    japanese\n    synonyms\n    kind\n    rating\n    score\n    status\n    episodes\n    episodesAired\n    duration\n    airedOn { year month day date }\n    releasedOn { year month day date }\n    url\n    season\n\n    poster { id originalUrl mainUrl }\n\n    fansubbers\n    fandubbers\n    licensors\n    createdAt,\n    updatedAt,\n    nextEpisodeAt,\n    isCensored\n\n    genres { id name russian kind }\n    studios { id name imageUrl }\n\n    related {\n      id\n      anime {\n        id\n        name\n      }\n      manga {\n        id\n        name\n      }\n      relationKind\n      relationText\n    }\n\n    screenshots { id originalUrl x332Url }\n\n    scoresStats { score count }\n\n    description\n    descriptionHtml\n    descriptionSource\n  }\n}"
    })
    headers = {
    ...
    try:
        ...
        return f"Successfully processed {len(data['data']['animes'])} anime records"

    except Exception as e:
        # Логируем ошибку для дебага
        import traceback
        traceback.print_exc()
        return f"Error processing anime data: {str(e)}"
